...
((Drupal, once) => {
  ...
  Drupal.behaviors.brainsumStarterkitLanguageDropdown = {
    attach(context) {
      const languageBlock = once('languageDropdown', '[data-drupal-selector="block-dropdown-language"]', context);
      if (languageBlock) {
        context.querySelectorAll('.js-block-dropdown-language .c-dropbutton__wrapper').forEach(el => init(el));
      }
    }
  };
})(Drupal, once);